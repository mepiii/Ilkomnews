...
const SolarSwitch = ({ isDark }) => {
  ...
  return (
    <motion.div animate={isDark ? 'checked' : 'unchecked'}>
      <motion.svg width="20" height="20" viewBox="0 0 25 25" fill="none">
        {/* Sun rays + circle */}
        {[
          ...
          <motion.path
            ...
            variants={{ checked: { scale: 0 }, unchecked: { scale: 1 } }}
            transition={{ duration }}
...
export const AnimatedThemeToggle = ({ className = '' }) => {
  ...
  return (
    <button
      onClick={toggleTheme}
      className={`p-2 rounded-lg hover:bg-black/[0.06] dark:hover:bg-white/[0.08] transition-all duration-200 cursor-pointer text-current ${className}`}
