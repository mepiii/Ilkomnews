...
const FlowButton = React.memo(function FlowButton({ text = 'Learn More', onClick, className = '' }) {
  return (
    ...
    >
      ...
      <ArrowRight className="absolute w-4 h-4 left-[-25%] fill-none z-[10] group-hover:left-4 transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)]" style={{ stroke: 'currentColor' }} />
      <span className="relative z-[10] -translate-x-3 group-hover:translate-x-3 transition-all duration-500 ease-out">{text}</span>
      ...
      <ArrowRight className="absolute w-4 h-4 right-4 fill-none z-[10] group-hover:right-[-25%] transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)]" style={{ stroke: 'currentColor' }} />
    </motion.button>
  )
})
export { FlowButton }
