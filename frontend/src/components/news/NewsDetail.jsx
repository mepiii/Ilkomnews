...
const NewsDetail = ({ news, relatedNews = [] }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      {/* Article Content */}
      <article className="rounded-xl p-6 md:p-10" style={{ background: 'var(--bg-card)' }}>
        ...
        {news.summary && (
          <div className="mb-8 border-l-4 rounded-r-xl p-5" style={{ borderColor: 'var(--accent)', background: 'color-mix(in srgb, var(--accent) 6%, transparent)' }}>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-semibold uppercase tracking-wider" style={{ color: 'var(--accent)' }}>Ringkasan</span>
            </div>
            <p className="text-sm leading-relaxed italic" style={{ color: 'var(--text-secondary)' }}>{news.summary}</p>
          </div>
        )}
        ...
        {/* Tags */}
        {news.tags?.length > 0 && (
          <div className="mt-8 pt-6 flex flex-wrap items-center gap-2" style={{ borderTop: '1px solid rgba(255,255,255,0.04)' }}>
        ...
        {/* Author */}
        <div className="mt-8 p-5 rounded-xl flex items-start gap-4" style={{ background: 'var(--bg-secondary)' }}>
          {news.author_image_url ? (
            <img src={news.author_image_url} alt={news.author} className="w-12 h-12 rounded-full object-cover shrink-0" />
          ) : (
          ...
          <div>
            ...
            {(news.author_position || news.author_institution) ? (
              <p className="text-xs mb-1.5" style={{ color: 'var(--text-muted)' }}>
                {news.author_position}{news.author_position && news.author_institution ? ' · ' : ''}{news.author_institution}
              </p>
            ) : (
        ...
        {/* Actions */}
        <div className="mt-6 pt-6 flex items-center gap-3" style={{ borderTop: '1px solid rgba(255,255,255,0.04)' }}>
          ...
          <div className="relative">
            ...
            {showShareMenu && (
              <>
                ...
                <div className="absolute right-0 mt-2 rounded-xl shadow-xl py-1.5 z-20 min-w-[180px]" style={{ background: 'var(--bg-secondary)' }}>
                  {[
                  ...
                  <div className="my-1" style={{ borderTop: '1px solid rgba(255,255,255,0.04)' }} />
        ...
        <div className="mt-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {relatedNews.slice(0, 3).map(item => (
              <Link key={item.id} to={`/news/${generateSlug(item.title)}`} className="group rounded-xl overflow-hidden transition-all hover:-translate-y-1" style={{ background: 'var(--bg-secondary)' }}>
