...
const AiProjectsTab = () => {
  ...
  useEffect(() => {
    projectsService.getAll({ category: 'ai' })
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 mb-4">
        <p className="text-theme-muted text-sm"><span className="font-semibold text-accent">{filteredProjects.length}</span> project AI / lainnya</p>
      </div>

      {filteredProjects.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {filteredProjects.map(p => <AiProjectCard key={p.id} project={p} />)}
        </div>
      ) : (
        <div className="text-center py-16 bg-theme-secondary rounded-2xl border border-theme">
          <div className="inline-flex items-center justify-center w-20 h-20 bg-accent/10 rounded-full mb-4"><Cpu size={40} className="text-accent" /></div>
